...
// 게시글보기 썸네일 생성
function get_view_thumbnail_wh($contents, $thumb_width=0, $thumb_height=0, $flag='')
{
    global $board, $config;
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if($flag == 'img') $contents = '<img src="'.$src.'" alt=""/>';
        ...
        if(is_file($srcfile)) {
            ...
            // 원본 width가 thumb_width보다 작다면
            if($size[0] <= $thumb_width && ($thumb_height > 0 && $size[1] <= $thumb_height)){
                if($flag == 'img') $contents = '<img src="'.$src.'" width="'.$size[0].'" alt=""/>';
                continue;
            }
            ...
            // 썸네일 높이
            $limit_thumb_height = $thumb_height;
            $thumb_height = round(($thumb_width * $size[1]) / $size[0]);
            if($thumb_height > $limit_thumb_height){
                $thumb_width = round($thumb_width * ($limit_thumb_height / $thumb_height));
                $thumb_height = $limit_thumb_height;
            }
            $filename = basename($srcfile);
            $filepath = dirname($srcfile);
            ...
            if($flag == 'img') $contents = $thumb_tag;
            else $contents = str_replace($img_tag, $thumb_tag, $contents);
        }
    }
